...
class ModelView_Pacient: ObservableObject{
    @Published var pacientes:[pacient] = []
    
    func fetch(){
        guard let url = URL(string: "http://127.0.0.1:1880/getpacient")else{
            return
        }
    ...
    func post(paciente: pacient){ //Aqui deve ser colocado o nome da Struct relacionada
        ...
        guard let url = URL(string: "http://127.0.0.1:1880/postpacient") else { return }
